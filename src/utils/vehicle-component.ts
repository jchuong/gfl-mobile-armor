...
export function isIdealComponent(component: VehicleComponent): boolean {
  const idealTalents = IDEAL_TALENTS_BY_COMPONENT[component.name];
  if (!idealTalents) {
    return false;
  }
  const rolls = [
...
export function convertRawToVehicleComponent(input: VehicleComponentWithUserInfo) {
  ...
  return componentList.map((component: RawVehicleComponent): VehicleComponent => {
    ...
    return {
      ...
      roll_1_raw: component.roll_1,
      roll_2_raw: component.roll_2,
      roll_3_raw: component.roll_3,
      roll_4_raw: component.roll_4,
      roll_5_raw: component.roll_5,
